...
import { useNEOFFI } from '../../../utils/fixedQuestionnaire';
...
export const NEOFFITestItem = () => {


  const { TextArea } = Input;

  const { isLoading, error, data: NEOFFI } = useNEOFFI();
  const instruction = NEOFFI?.instruction;
  const allTests = NEOFFI?.list;
  // const {instruction, list} = NEOFFI.data
      ...
      allTests?.map((test: FixedList, index: number) => <TestItemCard loading={isLoading} key={test.id}>
        {
          ...
          >
            <Radio.Group style={{ width: '100%' }}
              ...
              <Radio key={test.option5} value={5}>{test.option5}</Radio>
            </Radio.Group>
          </TestFormItem>
        }
      </TestItemCard>)
