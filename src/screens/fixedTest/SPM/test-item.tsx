...
import { useSPM } from '../../../utils/fixedQuestionnaire';
...
export const SPMTestItem = () => {


  const { TextArea } = Input;

  const { isLoading, error, data: SPM } = useSPM();
  const instruction = SPM?.instruction;
  const allTests = SPM?.list;
  // const {instruction, list} = SPM.data
