...
import { usePSSS } from '../../../utils/fixedQuestionnaire';
...
export const PSSSTestItem = () => {


  const { TextArea } = Input;

  const { isLoading, error, data: PSSS } = usePSSS();
  const instruction = PSSS?.instruction;
  const allTests = PSSS?.list;
  // const {instruction, list} = PSSS.data
