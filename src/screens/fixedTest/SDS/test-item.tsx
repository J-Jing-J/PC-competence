...
import { useSDS } from '../../../utils/fixedQuestionnaire';
...
export const SDSTestItem = () => {


  const { TextArea } = Input;

  const { isLoading, error, data: SDS } = useSDS();
  const instruction = SDS?.instruction;
  const allTests = SDS?.list;
  // const {instruction, list} = SDS.data
